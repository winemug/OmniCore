...
namespace OmniCore.Services
{
    public class Radio : IDisposable
    {
        ...
        private IService _mainService = null;
        ICharacteristic _dataCharacteristic = null;
        ICharacteristic _responseCountCharacteristic = null;
            ...
            try
            {
                while (true)
                {
                    ...
                    try
                    {
                        ...
                        await _connectionLostEvent.WaitAsync(cancellationToken);
                    }
                    catch (TaskCanceledException)
                    {
                        throw;
...
// 0x12 -30        
        ...
        private async Task TryWriteToCharacteristic(ICharacteristic characteristic, byte[] data, CancellationToken cancellationToken = default)
        {
            int retries = 0;
            var success = false;
            Exception ex = null;
